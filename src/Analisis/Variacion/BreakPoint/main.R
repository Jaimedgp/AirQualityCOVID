...
main <- function(dataFrame, periods) {
    ...
    #     Variacion Diarias
    #
    # Calcular la variacion relativa diaria de las concentraciones
    #-----------------------------------------------------------------------

    # Calcular variaciones
    var.med <- get.difference(data.all, mainCol=paste(end_dt, sep=""),
                                      restCol=c(paste(start_dt, "-",
                                                      end_dt-1, sep="")),
                             )
...
if(!interactive()) {
    ...
    periods <- list(
        # new year
        new.year=ymd_hms("2020-01-01 00:00:00"),
    ...
    if (group.by.municipio) {
        ...
        folder.name <- "Plots/Analisis/Variacion/BreakPoint/by_municipio"
        file.name <- "data/Analisis/Variacion/BreakPoint/variacion_media_municipios.csv"
    } else {
        file.name <- "data/Analisis/Variacion/BreakPoint/variacion_media.csv"
        folder.name <- "Plots/Analisis/Variacion/BreakPoint/by_site"
    }
    ...
    for (st in levels(as.factor(var.med$site))) {
